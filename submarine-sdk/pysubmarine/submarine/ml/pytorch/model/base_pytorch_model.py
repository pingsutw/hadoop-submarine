...
from submarine.ml.pytorch.utils import write_file
...
class BasePyTorchModel(AbstractModel, ABC):
    def __init__(self, params=None, json_path=None):
        ...
        print(self.params['model']['kwargs'])
        ...
        self.loss = get_loss_fn(key=self.params['criterion']['name'])(
            **self.params['criterion']['kwargs'])
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for ids_batch, batch in enumerate(valid_loader):
                sample, target = batch
                output = self.model(sample)
    ...
    def predict(self):
        ...
        with torch.no_grad():
            for ids_batch, batch in enumerate(test_loader):
                sample, target = batch
                output = self.model(sample)
                outputs.append(output)
        # TODO: fix this
