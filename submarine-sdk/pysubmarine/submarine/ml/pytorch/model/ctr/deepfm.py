...
class _DeepFM(nn.Module):
    def __init__(self, field_dims, embedding_dim, out_features, hidden_units, dropout_rates, **kwargs):
        super().__init__()
    ...
    def forward(self, x):
        ...
        return self.field_linear(x) + self.pairwise_interaction(emb)\
               + self.dnn(torch.flatten(emb, start_dim=1))
